...
wrapper <- function(x) {
  ...
  # Combine the groupings - the order demonstrated is arbitrary
  passages <- cbind(upstream_p, downstream_p, downstream_juvp)
  
  # Sample one row for each simulation
  passagex <- passages[sample(1:nrow(passages), 1), ]
...
Load libraries on cluster and define desired number of iterations. Note that `niterations <- 100` is used to obtain a minimal output necessary to generate a plot here for brevity. A larger simulation with 1,000 - 2,000 runs may take 20-40 minutes on a modern laptop depending on hardware, number of years simulated, and passage efficiencies (due influence on number of fish used on time to run individual-based model components).
